<template>
    <div>
        <zhuce></zhuce>
        <!--1、用户名输入框-->
        <mt-field label="用户名" :placeholder="unameholder" v-model="uname" :attr="{maxlength:12}" class="n_9"></mt-field>
        <!--2、密码输入框-->
        <mt-field label="密码" :placeholder="upwdholder" v-model="upwd" :attr="{maxlength:16}" type="password" class="n_9"></mt-field>
        <!--3、验证码输入框-->
        <mt-field label="验证码" v-model="yz" :placeholder="holder" @focus.native.capture="checkCard" class="n_9"></mt-field>
        <!--4、随机生成验证码-->

        <!--5、登录按钮-->
        <mt-button size="large" @click="reg" class="n_9">注册</mt-button>
