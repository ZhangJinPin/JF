    ...
    >
      ...
      >
        ...
        <div v-else class="everyDay">
          ...
          <div @click="getDayMessage(currentYear,currentMonth,dayobject.day.getDate(),index)" v-if="dayobject.day.getFullYear() == new Date().getFullYear()&& dayobject.day.getMonth() == new Date().getMonth()&& dayobject.day.getDate() == new Date().getDate()" id="act" class="">{{ dayobject.day.getDate() }}</div>
  ...
  methods: {
    getDayMessage(y, m, d, index) {
      ...
      if(this.count>0){
        ...
        // console.log(a);
        this.axios.get("qian",{params:{integral:20}}).then(res=>{
          if(res.data.code==-1){
            this.$toast("请先登录");
            this.$router.push("/Login");
          }else{
            this.$toast("签到成功,积分+20");
          }
        })
      }
    },
    ...
    allTouchStart: function(e) {
      ...
      this.positionSX = e.changedTouches[0].clientX; //开始y轴坐标
    ...
    allTouchEnd: function(e) {
      ...
      this.positionEX = e.changedTouches[0].clientX; //结束y轴坐标
      this.positionEY = e.changedTouches[0].clientY;

      var distanceY = this.positionEY - this.positionSY;

      var distanceX = this.positionSX - this.positionEX; //判断滑动的方向
    ...
    touchEnd: function(e) {
      this.show = !this.show; //结束x轴坐标
      this.positionEX = e.changedTouches[0].clientX; //结束y轴坐标
      this.positionEY = e.changedTouches[0].clientY;

      var distanceY = this.positionEY - this.positionSY;

      var distanceX = this.positionSX - this.positionEX; //判断滑动 的方向
...
#calendar {
  ...
  height: 666px;
  margin: 0 auto;
  background:url("../../../assets/qiandao.jpg");
