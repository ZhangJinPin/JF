...
server.get("/products",(req,res)=>{
    ...
    //创建sql语句
    var sql=`select pid,img,img2,img3,title,integral,frequency from integral_product where pid=?`;
...
server.get("/integral",(req,res)=>{
    ...
    var sql=`UPDATE integral_user SET integral=integral+1  WHERE uid=${uid}`;
    pool.query(sql,(err,result)=>{
        if(err)throw err;
        console.log(result);
...
server.get("/hqu",(req,res)=>{
    ...
    if(!uid){
        ...
        var sql=`SELECT uname,integral,uid FROM integral_user WHERE uid=?`;
        pool.query(sql,[uid],(err,result)=>{
            if(err)throw err;
            res.send({code:1,msg:"查询成功",data:result});
        });
...
server.get("/sample",(req,res)=>{
    ...
    //获取页面数据
    var integral=req.query.integral;
    var sql=`UPDATE integral_user SET integral=integral+? WHERE uid=${uid}`;
    pool.query(sql,[integral],(err,result)=>{
        if(err)throw err;
        res.send({code:1,msg:"修改成功",data:result});
    });
})
//http://127.0.0.1:3000/sample?integral=40

//功能签到加积分
server.get("/qian",(req,res)=>{
    //获取用户数据
    var uid = req.session.uid;
    if(!uid){
        //2.3:返回错误消息  
